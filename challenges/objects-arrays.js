...
function getNames(item){
  var bothNames = `Name: ${item.animal_name}, Scientific: ${item.scientific_name}`;
  displayNames.push(bothNames);
}
...
for(let animal of zooAnimals){
  populationCount.push(animal.population);
}
const populationTotal = populationCount.reduce((total, population) => total + population);
